...
q_table = np.zeros((grid_size * grid_size, 4), float) # cada casilla del mapa como fila, cada acción (dirección) como columna
learning_rate = 0.1 # 0 -> no se modifican los q-values ; 1 -> se modifican enteros
discount_factor = 0.9 # 0 -> miope ; 1 -> optimista
max_iterations = 500 # número de iteraciones del juego
for iteration in range(1, max_iterations + 1):
    ...
    while RUNNING:
        ...
        if random.random() < exploration_threshold:
            for pos_action in range(4): # intentamos explorar todas las casillas, busca una nunca accedida
